...
bool DisplayUI::backHolding(uint32_t time) {
    return backBtn && backBtn->holding(time);
}

void DisplayUI::setupButtons() {
    upBtn    = new ADCButton(BTN_UP, BTN_TOLERANCE);
    downBtn  = new ADCButton(BTN_DOWN, BTN_TOLERANCE);
    leftBtn  = new ADCButton(BTN_LEFT, BTN_TOLERANCE);
    rightBtn = new ADCButton(BTN_RIGHT, BTN_TOLERANCE);
    enterBtn = new ADCButton(BTN_ENTER, BTN_TOLERANCE);
    backBtn  = new ADCButton(BTN_BACK, BTN_TOLERANCE);

    // === BUTTON UP === //
    upBtn->setOnClicked([this]() {
        scrollCounter = 0;
    ...
    leftBtn->setOnClicked([this]() {
        ...
        buttonTime    = currentTime;
        if (!tempOff) {
            if (mode == DISPLAY_MODE::PACKETMONITOR) {
                scan.setChannel(wifi_channel - 1);
            } else if (mode == DISPLAY_MODE::CLOCK) {
                setTime(clockHour - 1, clockMinute, clockSecond);
            }
        }
    });

    leftBtn->setOnHolding([this]() {
        scrollCounter = 0;
        scrollTime    = currentTime;
        buttonTime    = currentTime;
        if (!tempOff) {
...
void DisplayUI::drawButtonTest() {
    drawString(0, str(D_UP) + b2s(upBtn->read()));
    drawString(1, str(D_DOWN) + b2s(downBtn->read()));
    drawString(2, str(D_LEFT) + b2s(leftBtn->read()));
    drawString(3, str(D_RIGHT) + b2s(rightBtn->read()));
    drawString(4, "ENTER:" + b2s(enterBtn->read()) + " BACK:" + b2s(backBtn->read()));
}
